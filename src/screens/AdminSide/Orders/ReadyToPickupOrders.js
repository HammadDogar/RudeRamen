...
const ReadyToPickupOrder = ({navigation}) => {
  const {user} = useContext(AuthenticationContext);
  const focus = useIsFocused();

  const [orderLoading, setOrderLoading] = useState(false);
  const [readyToPickupOrders, setReadyToPickupOrders] = useState([]);

  useEffect(() => {
    setOrderLoading(true);
    getOrdersByresturantUserIdAndStatusId(
      user.uid,
      ORDER_STATUS_IDS.READY_TO_PICK_UP,
    )
      .then(response => {
        setOrderLoading(false);
        setReadyToPickupOrders(response.reverse());
      })
      .catch(() => {
        setOrderLoading(false);
    ...
    <SafeAreaView style={styles.container}>
      <View style={{flex: 1}}>
        <ScrollView
          ...
          {!orderLoading && readyToPickupOrders.length > 0 && (
            <View>
              {readyToPickupOrders?.map((itemData, index) => (
                <ItemCard
                  title={'tittle'}
...
export default ReadyToPickupOrder;
